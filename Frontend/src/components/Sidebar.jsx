...
const Sidebar = () => {
  return (
    <>
      ...
      {/* Mobile/tablet bottom tab bar — visible below md, fixed to viewport bottom */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-ink border-t border-white/10 flex items-stretch justify-around px-1 pb-[env(safe-area-inset-bottom)]">
        {links.map(({ to, label, icon: Icon }) => (
          <NavLink
            key={to}
            to={to}
            ...
            className={({ isActive }) =>
              `flex-1 flex flex-col items-center justify-center gap-1 py-2.5 text-[11px] transition-colors ${
                isActive ? 'text-cobalt-500' : 'text-slate-400'
              }`
            }
          >
            {({ isActive }) => (
